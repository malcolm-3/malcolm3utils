...
class FilterParser(Transformer):
    ...
    def or_test(self, *args):
        ...
        return lambda d: any([a(d) for a in args])

    def and_test(self, *args):
        logger.debug("And test: %s", args)
        return lambda d: all([a(d) for a in args])
    ...
    def key(self, a):
        ...
        try:
            ...
            logger.exception("Unable to locate data column: '%s'", a)
...
def create_filter(filter_spec: str) -> Callable[[dict[str, Any]], bool]:
    """
